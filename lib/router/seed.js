...
router.put('', user.login.require, async (req, res)=>{
    ...
    seeds.forEach(s => {
        ...
        assert.ok(!s.time.happen || typeof s.time.happen.timestamp === 'number', 'time.happen.timestamp must be a number');
        if(s.tags) {
...
router.get('', async (req, res)=>{
    // todo 优化翻页查询
    let {limit = 20, page = 1, /*last_uri = '',*/sort = 'uri', ending = 'desc', tags = '', mode = 'or'} = req.query;
    limit = parseInt(limit);
    tags = tags.split(',').filter(t=>t.length);
    page = parseInt(page);
    assert.ok(['asc', 'desc'].includes(ending), 'ending must be asc or desc');
    assert.ok(['uri', 'time.happen.timestamp', 'time.create', 'time.update', 'time._publish'].includes(sort), 'sort must be uri, time.happen.timestamp, time.create, time.update, time._publish');
    ...
    assert.ok(['and', 'or'].includes(mode), 'mode must be and or or');
...
router.delete('/:b64_uri', user.login.require, async (req, res)=>{
    const uri = Buffer.from(req.params.b64_uri, 'base64').toString('utf8');
    await mongo.db.collection('seed').deleteOne({uri, _publisher: {$eq: req.session.username}});
    ...
    async router () {
        ...
        if (list.length === 0) {
            await (await mongo.db.createCollection('seed')).createIndexes([
                ...
                {key: {'time.happen.timestamp': 1}, unique: false},
