...
router.put('', user.login.require, async (req, res)=>{
    ...
    seeds.forEach(s => {
        ...
        if(s.tags) {
            assert.ok(Array.isArray(s.tags) && s.tags.length, 'tags must be an array');
            s.tags.forEach(t => {
...
router.get('', async (req, res)=>{
    ...
    assert.ok(['uri', 'time.create', 'time.update', 'time._publish'].includes(sort), 'sort must be uri, time.create, time.update or time._publish');
